...
read numExecutors
echo "The max number of core in each executor:"
read executorCores
...
function run()
{
	for ((cpt_core=1;cpt_core<=$executorCores;cpt_core++));do
		...
		for((i=1;i<=$numExecutors;i++));do
			...
			cmd="spark-submit --class projet.Bigram --master yarn-cluster --conf spark.default.parallelism=$nb --num-executors $i --executor-cores $cpt_core  ~/spark-self-cibtained-project_2.10-1.0.0.jar >tmp 2>&1&&cat tmp|grep proxy/application_[0-9]*_[0-9]*|uniq|sed -e 's/\//\n/g'|grep application>appId"
